          ...
          <div className="grid gap-4 lg:col-span-5">
            {mid.map((item, i) => (
              <Reveal key={item.title} delay={0.1 + i * 0.05}>
                <article className="hover-lift hover-zoom group flex h-full flex-row items-stretch overflow-hidden rounded-sm border border-bone-line bg-bone hover:border-ink/25 sm:gap-5">
                  <Figure
                    slot={item.image}
                    ratio="1/1"
                    className="w-[7.25rem] shrink-0 rounded-none border-r border-bone-line sm:w-[9rem]"
                    sizes="(max-width: 640px) 30vw, 12vw"
                    tone="bone"
                    showNote={false}
                  />
                  <div className="flex min-w-0 flex-1 flex-col justify-center p-4 sm:py-5 sm:pl-0 sm:pr-6">
