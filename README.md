...
Card images carry two ratios. `Figure` takes `ratioMobile` alongside `ratio`,
because a 4/3 image that looks right in a desktop grid is 250px of dead height
on a 375px phone. Measured on the homepage at 375px:

| | Before | After |
|---|---|---|
| Tallest card | 493px (61% of viewport) | 391px (48%) |
| Cards over 55% viewport | 6 | 0 |
| Cardiovascular Training | 493px | 177px |
| Post-Workout Nutrition | 448px | 172px |
| Page height | 23,315px (28.7 screens) | 20,120px (24.8) |

`MediaCard` is the shared numbered image + title + body card used by nutrition,
recovery and the training grid. Below `sm` it lays out horizontally with a
compact square thumbnail; from `sm` up it returns to the stacked composition
the desktop grid was designed around. One component, so padding, borders and
the numbering treatment stay identical everywhere.

The type scale clamps now have a real mobile end — `display-sm` bottomed out at
22px, which made card headings dominate a phone-width card. Section padding was
~2000px of pure whitespace across 14 sections on a phone; its floor came down
too.

Verified at 320 / 375 / 430 / 768 / 1440: no horizontal overflow, no text
clipped by an overflow container, contrast passes AA across 236 nodes.

---

## Social preview

`src/app/opengraph-image.png` is a **static** 1200x630 file generated by
`scripts/generate-og-image.py` from the real logo and a real photograph.

```bash
python scripts/generate-og-image.py
```

It used to be a runtime `next/og` ImageResponse. WhatsApp and iMessage are
unreliable with those — their crawlers have short timeouts and often fall back
to no preview at all. A plain file is fetched once and cached by every scraper.

Next's file convention wires it to both `og:image` and `twitter:image` with
explicit width, height, type and alt. `manifest.ts` covers the PWA basics, and
`icon.png` / `apple-icon.png` the tab and home-screen icons.
...
- **LQIP placeholders.** `scripts/generate-blur.py` emits an 8px base64
  placeholder per photograph into `src/lib/blur.ts`, which `Figure` passes as
  `blurDataURL`. Images no longer pop in as they decode — that pop-in was the
  main thing that read as slow loading. `Figure` is a server component, so only
  the placeholders actually used on a page reach the client.
- **Logo sources cut from 2.0MB to 816KB.** They are photographic PNGs on an
  opaque black field, rendered at 46-520px; the originals made the first
  optimisation pass needlessly slow.
